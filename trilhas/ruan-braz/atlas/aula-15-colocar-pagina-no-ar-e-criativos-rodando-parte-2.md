...
- Identificar as métricas centrais de desempenho de um anúncio nas primeiras horas de veiculação e extrair conclusões práticas a partir delas
- Aplicar a lógica de previsibilidade de custo por clique para estimar o volume de leads que um investimento definido pode gerar
- Distinguir os diferentes destinos de um anúncio (formulário de página, WhatsApp direto e pesquisa de diagnóstico) e reconhecer o impacto de cada rota no funil de validação
...
A lógica apresentada é a seguinte: se cada clique custou R$ 0,10 e o orçamento da campanha era de R$ 112,00 distribuídos ao longo de cinco dias, é possível calcular que, mantendo esse custo médio, a campanha geraria aproximadamente 1.120 cliques. Isso equivale a dez cliques por real investido, ou 100 cliques para cada R$ 10,00 gastos.
...
Os fatores que ele chama de "invisíveis" são:

- **Oferta com demanda real.** Antes de criar o anúncio, Ruan passou um dia analisando dados globais de mercado para identificar uma dor com audiência real. Quem pula essa etapa e testa qualquer oferta sem análise prévia tende a pagar mais caro por clique, simplesmente porque a mensagem não ressoa com ninguém específico.

- **Cópia que fala com a dor.** A frase do anúncio em questão foi: "Escale seu conteúdo sem virar refém da operação e sem queimar sua reputação." Essa formulação funciona porque nomeia dois medos simultâneos do público-alvo: a perda de controle sobre o próprio processo e o risco à reputação ao tentar escalar com IA. Quem configura o gerenciador corretamente mas coloca uma mensagem genérica não obtém o mesmo resultado, porque são as palavras que fazem a pessoa clicar, não a técnica de configuração.
...
Em uma aula de prototipagem, o desafio era criar uma embalagem de ovo que sobrevivesse a uma queda de dois metros de altura sem quebrar nenhum ovo. Os materiais permitidos eram baratos: papelão, folhas de ofício e outros insumos simples. A proposta pedagógica era clara: antes de investir em material nobre, valide a forma. O protótipo de papelão que funcionou não era bonito, mas cumpria o requisito.
...
A lógica é simples. Com dez criativos e o mesmo orçamento, o custo total no primeiro dia seria de aproximadamente R$ 8,00. Cada criativo receberia uma fração do investimento e geraria seus próprios dados. A partir desse conjunto, é possível identificar quais promessas e quais imagens geram mais cliques, e direcionar mais investimento para os criativos vencedores.

Além de testar variações visuais e de mensagem, Ruan propõe também testar diferentes destinos para o anúncio. Ele sugere dividir os dez criativos em três grupos:

- **Três criativos apontando para o formulário no site.** O lead preenche dados e entra no processo de qualificação antes de receber contato.
- **Três criativos apontando para o WhatsApp direto.** O lead inicia uma conversa imediata, sem formulário intermediário.
...
Ele descarta o CRM avançado nessa fase porque a configuração de uma ferramenta profissional leva tempo, e o objetivo agora é validar, não otimizar processos. A planilha serve como CRM de prototipagem.

A estrutura recomendada para a planilha inclui as seguintes colunas:

- **Nome** do lead
- **E-mail**
- **WhatsApp** (ponto de contato principal)
- **Origem** do lead (de qual criativo ou canal ele veio)
- **Destino** (se foi para o formulário, WhatsApp direto ou pesquisa)
- **Link do criativo** que gerou o clique (para referência visual futura)
- **Estágio** atual no funil (novo, contatado, respondeu, agendou call, apareceu, fez proposta, fechou, perdido)
...
A primeira ação é ligar duas vezes para o lead. A lógica por trás dessa recomendação é que a pessoa pode estar longe do telefone na primeira chamada. Ligar uma segunda vez consecutiva sinaliza urgência e aumenta as chances de atendimento. Antes de atender, a pessoa não sabe quem está ligando, então o risco de queimar reputação é nulo: ela simplesmente vê um número desconhecido e pode desligar se quiser.

Se atender, o script inicial segue esta estrutura:

1. Apresentar-se e reconhecer que a ligação pode ser inesperada
2. Mencionar que a pessoa se cadastrou no site ou clicou no anúncio e estava interessada em resolver o problema específico mencionado na promessa
3. Fazer uma pergunta aberta para iniciar a conversa, como: "Você já trabalha com produção de conteúdo hoje?"
4. Escutar a resposta e, com base nela, posicionar a solução brevemente
5. Propor agendar uma call para aprofundar o assunto

Se a ligação não for atendida, a próxima ação é enviar uma mensagem no WhatsApp. Se o lead já chegou pelo WhatsApp, o desenvolvimento começa direto na conversa.
...
1. Lead clica no anúncio e cai em um dos três destinos (formulário, WhatsApp ou diagnóstico)
2. Lead entra na planilha com dados de origem e destino
3. SDR (ou o próprio fundador, no início) inicia o fluxo de contato: ligação, mensagem, desenvolvimento no WhatsApp
4. Lead responde e tem perfil qualificado? Avança para o agendamento de call
5. Lead responde mas não tem perfil? É movido para a lista de descarte
6. Lead não responde após seis dias? Mensagem final e descarte
7. Call agendada: lead aparece (show)? Apresentação da proposta
8. Lead não aparece (no-show)? Tentativa de reagendamento por follow-up
9. Lead na call: aceita a proposta? Venda fechada
10. Lead na call: não aceita? Entra em fluxo de follow-up pós-call com tentativa de reagendar ou resolver objeção
