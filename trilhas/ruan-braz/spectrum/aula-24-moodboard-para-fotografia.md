# Moodboard para Fotografia

**Tempo estimado de leitura:** 18 minutos

## Objetivos de aprendizado

Ao final desta aula, você será capaz de:

- Estruturar um moodboard fotográfico a partir de palavras-chave extraídas do briefing, usando o Pinterest como plataforma de curadoria
- Aplicar critérios de seleção intencional de referências fotográficas, reconhecendo o que serve e o que distorce o resultado da IA
- Executar a montagem de painéis separados no Figma para figurino e fotografia, mantendo coerência entre os elementos
- Identificar cuidados práticos ao usar referências de outras pessoas para evitar mistura indesejada de traços físicos pela IA
- Distinguir a diferença que a qualidade e a variedade das fotos de referência própria fazem no resultado final gerado

## O ponto de partida: quantidade e qualidade nas fotos de referência

A aula começa de onde a anterior terminou: com um conjunto inicial de referências já montado no Figma, mas ainda com apenas 9 imagens. Ruan faz a observação imediata: o ideal é ter mais imagens. Pelo menos 15 ou 20 imagens de referência vão ajudar no resultado e vão fazer diferença. Quantidade de referência não é capricho, é estratégia para dar à IA mais variáveis para trabalhar.

A orientação é clara: se você não tem todas essas imagens para trabalhar, vá com o que tem. Mas nunca deixe de ampliar o conjunto quando possível.

## Qualidade e ângulo das fotos de referência própria

Uma parte significativa da aula aborda como tirar fotos de referência de si mesmo para alimentar a IA. Esse processo tem detalhes técnicos importantes que afetam diretamente a qualidade do resultado gerado.

Ruan recomenda usar a câmera traseira do celular em vez da frontal. O motivo é técnico: a câmera traseira tem maior resolução e distorce menos o rosto. A câmera frontal, especialmente em zoom mais fechado, pode criar distorções que a IA vai replicar. Se a IA tem como referência um rosto distorcido pela câmera frontal, ela pode gerar o rosto com essas mesmas distorções.

A variação é fundamental: mudar o cenário, mudar a iluminação, mudar a pose. Cada variação dá à IA mais informação sobre como o sujeito se parece em condições diferentes, tornando o resultado final mais versátil e preciso. Tirar fotos ao sol, em ambiente externo, além da iluminação artificial interna, também é uma boa prática. Trocar de roupa entre as fotos é recomendado.

É importante ter fotos do corpo inteiro ou pelo menos do tronco, não apenas do rosto. A IA precisa ter noção das proporções corporais para gerar imagens mais precisas. Se só há fotos do rosto, a IA vai ter que imaginar o restante, o que aumenta a probabilidade de inconsistências.

A luz importa muito: se a luz está vindo muito de cima, ela cria sombras duras abaixo dos olhos e do nariz. A IA pode replicar isso em posições que não fazem sentido no contexto gerado. Fotos com iluminação mais controlada e frontal dão resultados mais consistentes.

## Como tirar fotos de referência na prática

Ruan demonstra o processo ao vivo durante a aula: tira fotos de si mesmo usando o celular, registra em diferentes ângulos e câmeras, avalia a qualidade de cada uma e descarta as que não servem.

Fotos em que o cabelo estava saindo de baixo do boné, por exemplo, foram descartadas: a IA pode replicar esses "erros" visuais como características permanentes do sujeito, gerando imagens inconsistentes com o que se quer construir. A recomendação é simples: use fotos boas. Se uma foto não ficou bem enquadrada, tire de fora da seleção.

Para quem não consegue tirar as próprias fotos com qualidade, a solução é pedir para alguém ajudar: fazer um ensaio caseiro mais básico, com alguém segurando o celular, em fundo branco, com cenário simples, com iluminação natural do sol. A qualidade não precisa ser profissional para servir como referência inicial, mas precisa ser funcional.

Fotos de ensaios profissionais, quando disponíveis, fazem muita diferença para a IA. No caso da aula, algumas fotos do ensaio feito com o fotógrafo Daniel Meg são incluídas no moodboard, e Ruan observa a diferença de qualidade visível em comparação com as selfies tiradas durante a live.

## Enviando fotos do celular para o Figma

O processo prático de transferência das fotos do celular para o Figma é demonstrado na aula: Ruan se envia as fotos via WhatsApp Web e depois arrasta do navegador para o arquivo do Figma. É um fluxo simples e funcional para quem não tem um sistema mais elaborado de transferência de arquivos.

A observação sobre o WhatsApp é direta: Ruan tem múltiplas contas Google, o que gera um pouco de confusão no processo, mas serve para mostrar que o workflow real tem pequenas fricções que são normais.

## Construindo o moodboard no Figma: dois painéis distintos

A organização do moodboard no Figma segue uma lógica de separação por tipo de referência. Ruan cria dois grupos distintos:

Moodboard Outfit: concentra todas as referências de figurino, roupas e acessórios. Esse painel organiza o estilo de vestimenta que a IA deve usar como base para representar o sujeito.

Moodboard Foto: concentra as referências de composição fotográfica, iluminação, ângulo e atmosfera. Esse painel orienta como as imagens devem ser construídas visualmente, independentemente de quem está sendo fotografado.

Essa separação é importante por um motivo prático: evita que a IA misture referências de composição com referências de figurino de forma não intencional. Se todas as referências estão em um único painel, a IA pode interpretar elementos de figurino de uma imagem como elementos composicionais de outra, gerando resultados confusos.

## Pinterest como ferramenta de curadoria de referências de figurino

Para o moodboard de figurino, Ruan navega no Pinterest usando referências que havia criado anteriormente em colaboração com a Carol Volpe, designer de moda que faz parte da comunidade Vanguarda da Overlens. Eles criaram juntos um conjunto de referências de roupas experimentais e diferenciadas para explorar possibilidades de visual.

O processo de seleção das referências de figurino é intencional: Ruan vai arrastando imagens do Pinterest diretamente para o Figma, escolhendo peças que ressoam com o estilo que quer explorar. Alguns critérios de seleção são explicitados durante a navegação:

Prefere coisas que incomodam um pouco, que saem do lugar-comum. Gosta de peças com detalhes tipográficos, como uma camisa com "OVER" estampado. Peças que remetem a work in progress ou processo criativo são particularmente interessantes. Jaquetas com design diferenciado entram na lista. Roupas verdes que ele mesmo chegou a usar em eventos ao vivo também são referência.

A instrução sobre tamanho das imagens é importante: não aumentar as imagens no Figma além do tamanho original. Ampliar a imagem distorce a resolução e pode causar artefatos que a IA vai replicar no resultado. A recomendação é manter as imagens no tamanho em que chegam, mesmo que os tamanhos variem entre si.

## Pinterest como ferramenta de curadoria de referências fotográficas

Para o moodboard fotográfico, Ruan usa as palavras-chave extraídas do briefing na aula anterior como guia de busca. O processo é mostrado ao vivo: diferentes combinações de palavras-chave geram diferentes tipos de resultados no Pinterest, e Ruan vai filtrando o que serve e o que não serve.

Algumas observações importantes durante a curadoria:

Referências de preto e branco são ricas para composição e luz, mesmo que o resultado final vá ser colorido. A composição e a qualidade da luz são transferíveis entre escalas de cor.

A foto do Mano Brown que aparece, com foco nas mãos, é citada como referência muito boa de conceito e intenção, mesmo sem copiar a identidade visual específica do artista.

O fotógrafo Platon é mencionado como referência importante de composição. Muita gente replica o estilo de Platon sem saber, porque suas fotografias se tornaram referências visuais amplamente compartilhadas. Buscar "Platon photographer" no Pinterest traz exemplos claros de sua abordagem: composições diretas, fundo neutro ou simples, foco total no sujeito, iluminação que esculpe sem achatar.

Willem Dafoe aparece como exemplo de fotografia de retrato com presença visual intensa: a foto específica mencionada é descrita como "classuda demais" e entra no moodboard.

A recomendação é misturar referências: ter muito preto e branco e também pegar um pouco de cor. Iluminações diferentes ampliam o vocabulário visual que a IA vai ter para trabalhar.

## Cuidados específicos com referências de outras pessoas

A aula traz um alerta prático importante que todo aluno precisa assimilar: quando você usa referências fotográficas de outras pessoas, a IA pode misturar o rosto dessas pessoas com o rosto do sujeito que está sendo gerado.

Isso é especialmente problemático quando a referência inclui pessoas de características físicas muito distintas do sujeito, ou quando as poses são muito específicas de uma fisionomia particular. Por exemplo: se você usa uma referência de alguém que usa óculos e a IA interpreta os óculos como um traço permanente do sujeito, ela vai gerar imagens com óculos mesmo que o sujeito não use.

O mesmo vale para botas, estilos de cabelo exagerados ou qualquer elemento muito marcante na referência. A IA não separa automaticamente "isso é referência de composição" de "isso é referência de aparência física". Quanto mais explícito você for no prompt sobre o que quer aproveitar de cada referência, melhor.

Para mitigar esse risco, Ruan sugere incluir o gênero nas palavras-chave de busca: ao pesquisar por "conceitual portrait", resultados com o gênero do sujeito aparecem com mais frequência, reduzindo a chance de referências com fisionomias muito diferentes serem incluídas.

## A lógica da busca por palavras-chave no Pinterest

Ruan demonstra que diferentes palavras-chave no Pinterest geram resultados muito diferentes, e que a estratégia de busca importa. Algumas observações do processo ao vivo:

Palavras-chave muito amplas, como "MEO" (abreviação usada para "men" em inglês), podem trazer resultados indesejados. Ruan aprende isso durante a aula e ajusta a busca para termos mais específicos como "conceptual portrait".

Buscar em inglês e em português pode gerar resultados distintos. Vale experimentar ambos para ampliar o leque de referências.

Palavras-chave compostas como "home office designer" ou "bookshelf portrait" trazem resultados mais contextuais e próximos do que se quer construir.

Para ambiente e cenário, palavras como "bookshelf portrait", "library background", "moody bookshelf" e "little bookshelf" trazem variações interessantes do tipo de ambiente que o briefing descreve: uma estante de livros como elemento composicional de autoridade, mas sem ficar grandioso ou distante demais das pessoas.

## A estante de livros como signo visual de autoridade

Um elemento recorrente ao longo de toda a curadoria é a estante de livros. A razão é conceitual e semiótica: a estante de livros é um signo visual de autoridade intelectual que aparece claramente no painel semântico construído na aula anterior. Ela não é apenas um fundo bonito: é um elemento que atesta etos, conhecimento acumulado e método de fundamentos.

As variações de busca exploram diferentes formas de trabalhar esse signo: estante grande no fundo (mais formal e imponente), estante menor e mais próxima (mais acolhedora e acessível), estante com iluminação mais quente (sensação de conforto), estante com baixa exposição (sensação de profundidade e mistério). Cada variação transmite uma versão ligeiramente diferente do mesmo signo.

O princípio aplicado aqui é que quando você identifica os signos que quer usar, você também pode controlar as nuances de como eles aparecem. O briefing e o painel semântico do processo anterior não eram exercícios teóricos: eles são o guia prático de todas essas decisões de curadoria.

## A diferença está na referência, não na ferramenta

Um dos momentos mais diretos da aula é quando Ruan para e faz uma afirmação clara: "Vocês vão ver que a diferença é a referência, não é a ferramenta." Essa frase sintetiza a filosofia que permeia toda a trilha.

Qualquer ferramenta de geração de imagem, seja qual for, vai produzir resultados muito mais precisos e coerentes quando alimentada com referências bem curadas do que quando recebe apenas um prompt de texto. A ferramenta não compensa a ausência de referência. A referência eleva o resultado de qualquer ferramenta.

Isso também significa que gastar tempo no Pinterest, no Figma ou em qualquer outro ambiente de curadoria não é perda de tempo: é investimento direto na qualidade do que vai ser gerado. Cada imagem bem selecionada no moodboard é uma decisão que a IA não vai precisar adivinhar.

## Quantidade e curadoria no moodboard final

Ao final da montagem, o moodboard criado durante a aula tem referências de figurino, referências fotográficas em preto e branco, referências de composição colorida, referências de ambiente com estante de livros e variações de iluminação. Ruan reconhece que o moodboard poderia ser ainda maior, mas que o que foi construído já tem referências legais suficientes para iniciar a geração.

A instrução final sobre quantidade de imagens no moodboard é: mais é melhor, até certo ponto. Ter 20 a 30 referências bem curadas é muito melhor do que ter 5 referências aleatórias. Mas a curadoria importa: não é para jogar tudo que aparece na tela. É para selecionar o que tem sinergia com o briefing e com o painel semântico.

Algumas referências que apareceram durante a busca no Pinterest foram explicitamente descartadas:

Imagens de poses sensuais, mesmo que a iluminação fosse boa, foram descartadas porque não é a mensagem que o briefing quer construir.

Imagens com muita retoque digital ou com estética "plastificada" foram descartadas porque contradizem o signo de autoridade autêntica mapeado no painel semântico.

Imagens com óculos, botas ou outros elementos que o sujeito não usa foram descartadas para evitar que a IA incorpore esses elementos como traços permanentes.

## O moodboard como linguagem compartilhada

O moodboard não é apenas uma coleção de imagens bonitas. É uma linguagem compartilhada entre o criador e a IA: um conjunto de decisões visuais que comunicam, de forma visual e direta, o que o briefing e o painel semântico descrevem em texto. Enquanto o painel semântico fala em signos e conceitos, o moodboard mostra exemplos concretos de como esses signos se materializam em fotografia real.

Juntos, o briefing estruturado, o painel semântico e o moodboard formam o tripé que vai sustentar toda a geração de imagens nas aulas seguintes. Cada elemento do tripé tem uma função específica: o briefing define o contexto estratégico, o painel semântico mapeia os signos e seus significados, e o moodboard traduz tudo isso em referência visual concreta.

## Coloque em prática

Usando as palavras-chave do briefing fotográfico que você montou na aula anterior, crie dois painéis separados no Figma:

1. Moodboard Outfit: busque no Pinterest pelo menos 8 referências de figurino que tenham sinergia com a estética do seu briefing. Use as palavras-chave de estilo de vestimenta geradas pelo LLM. Não aumente as imagens além do tamanho original para evitar distorção.

2. Moodboard Foto: busque no Pinterest pelo menos 12 referências de composição fotográfica, iluminação e ambiente. Use as palavras-chave de iluminação e estilo fotográfico do briefing. Misture preto e branco com referências coloridas. Inclua variações de ambiente relacionadas ao signo visual mais importante do seu painel semântico.

Tente também tirar pelo menos 5 fotos de referência de si mesmo em condições diferentes: câmera traseira, câmera frontal, iluminação natural, iluminação artificial, diferentes ângulos. Avalie cada uma e selecione apenas as que têm iluminação funcional e enquadramento adequado. Adicione essas fotos a uma terceira área no Figma, separada dos dois painéis, para que a IA tenha referência de quem você é.
